...
; This file was generated Sun May 01 17:26:12 2016
;--------------------------------------------------------
	.module hw4
...
;------------------------------------------------------------
;	hw4.c:45: void Main (void) __naked {
;	-----------------------------------------
;	 function Main
;	-----------------------------------------
_Main:
;	naked function: no prologue.
;	hw4.c:47: int iterator = 0;
	clr	a
	mov	_Main_iterator_1_27,a
	mov	(_Main_iterator_1_27 + 1),a
;	hw4.c:48: unsigned char plus = 1;
	mov	_Main_plus_1_27,#0x01
;	hw4.c:49: unsigned char first[] ={0,0,0,0,0,0};
...
;	hw4.c:50: unsigned char second[] ={0,0,0,0,0,0};
...
;	hw4.c:51: unsigned char final[] = {0,0,0,0,0,0};
...
;	hw4.c:53: unsigned char gotkey = 0;
	mov	r4,#0x00
;	hw4.c:59: unsigned char keypad[4][3] = {   // This array contains the ASCII of the keypad keys
...
;	hw4.c:68: functionSet();
	push	ar4
	lcall	_functionSet
;	hw4.c:69: entryModeSet(); // increment and no shift
	lcall	_entryModeSet
	pop	ar4
;	hw4.c:70: displayOnOffControl(1, 1, 1); // display on, cursor on and blinking on
...
;	hw4.c:74: while(1){
00116$:
;	hw4.c:75: row = 0xf7;  // The first row (connected to P0.3) will be zero
	mov	r3,#0xF7
;	hw4.c:77: for (i=0;i<4;++i){ // loop over the 4 rows
	mov	r2,#0x00
	mov	r5,#0x00
00127$:
;	hw4.c:79: P0 = 0xff;     // Initialize the 4 rows to '1' and set the column ports to inputs
	mov	_P0,#0xFF
;	hw4.c:80: P0 = P0 & row; // clear one row at a time
	mov	a,r3
	anl	_P0,a
;	hw4.c:81: col = P0 & 0x70;  // Read the 3 columns
	mov	a,#0x70
	anl	a,_P0
	mov	r7,a
;	hw4.c:82: if (col != 0x70){ // If any column is zero i.e. a key is pressed
	cjne	r7,#0x70,00172$
	sjmp	00102$
00172$:
;	hw4.c:83: col = (~col) & 0x70;  // because the selected column returns zero
	mov	a,r7
	cpl	a
	mov	r6,a
	mov	a,#0x70
	anl	a,r6
;	hw4.c:84: col = col >> 5;       // The column variable now contain the number of the selected column
	swap	a
	rr	a
	anl	a,#0x07
	mov	r7,a
;	hw4.c:85: gotkey = keypad[i][col]; // Get the ASCII of the corresponding pressed key
	mov	a,r2
	mov	b,#0x03
	mul	ab
...
;	hw4.c:86: big_delay();
	...
	lcall	_big_delay
	pop	ar4
;	hw4.c:87: break;  // Since a key was detected -> Exit the for loop
	sjmp	00103$
00102$:
;	hw4.c:89: row = row >> 1;   // No key is detected yet, try the next row
	mov	a,r3
	clr	c
	rrc	a
	mov	r3,a
;	hw4.c:90: row = row | 0xf0; // Only one of the least 4 significant bits is '0' at a time 	
	orl	ar3,#0xF0
;	hw4.c:77: for (i=0;i<4;++i){ // loop over the 4 rows
	inc	r5
	mov	ar2,r5
...
;	hw4.c:104: if (gotkey == '#'){
	cjne	r4,#0x23,00107$
;	hw4.c:106: sendChar(gotkey);
	mov	dpl,#0x3D
	lcall	_sendChar
;	hw4.c:107: break;
	sjmp	00117$
00107$:
;	hw4.c:111: if (gotkey != 0){
	mov	a,r4
	jz	00116$
;	hw4.c:112: if(plus == 0 ) {
	mov	a,_Main_plus_1_27
	jnz	00111$
;	hw4.c:113: second[iterator] = gotkey;
	mov	a,_Main_iterator_1_27
	add	a,#_Main_second_1_27
	mov	r0,a
	mov	@r0,ar4
;	hw4.c:114: iterator = iterator+1;		
	...
	sjmp	00112$
00111$:
;	hw4.c:117: else if (plus == 1) {
	mov	a,#0x01
	cjne	a,_Main_plus_1_27,00112$
;	hw4.c:118: first[iterator] = gotkey;
	mov	a,_Main_iterator_1_27
	add	a,#_Main_first_1_27
	mov	r0,a
	mov	@r0,ar4
;	hw4.c:119: iterator = iterator+1;
	inc	_Main_iterator_1_27
	clr	a
...
;	hw4.c:122: gotkey =0; 	
	...
	ljmp	00116$
00117$:
;	hw4.c:127: hmm = atoi(first) + atoi(second);
...
;	hw4.c:133: inter = (rem) + 48; 
	...
	addc	a,r3
	mov	r3,a
;	hw4.c:134: final[iterator] = inter; 
	mov	a,r4
	add	a,#_Main_final_1_27
	mov	r1,a
	mov	@r1,ar2
;	hw4.c:135: iterator = iterator+1;
	inc	r4
;	hw4.c:138: while (iterator > -1){
	cjne	r4,#0x00,00118$
	inc	r5
	sjmp	00118$
00142$:
...
00121$:
	...
	subb	a,b
	jnc	00125$
;	hw4.c:139: sendChar(final[iterator]);
	mov	a,r6
	add	a,#_Main_final_1_27
	mov	r1,a
	mov	dpl,@r1
...
;------------------------------------------------------------
;	hw4.c:149: void returnHome(void) {
;	-----------------------------------------
;	 function returnHome
;	-----------------------------------------
_returnHome:
...
;	hw4.c:150: RS = 0;
	clr	_P3_3
;	hw4.c:151: P1 = 0x02; // LCD command to return home (the first location of the first lcd line)
	mov	_P1,#0x02
;	hw4.c:152: E = 1;
	setb	_P3_2
;	hw4.c:153: E = 0;
	clr	_P3_2
;	hw4.c:154: big_delay(); // This operation needs a bigger delay
	ljmp	_big_delay
;------------------------------------------------------------
;Allocation info for local variables in function 'entryModeSet'
;------------------------------------------------------------
;	hw4.c:157: void entryModeSet() {
;	-----------------------------------------
...
;------------------------------------------------------------
;	hw4.c:165: void displayOnOffControl(__bit display, __bit cursor, __bit blinking) {
;	-----------------------------------------
...
;	hw4.c:170: P1_3 = 1;
	setb	_P1_3
;	hw4.c:171: P1_2 = display;
	mov	c,_displayOnOffControl_PARM_1
	mov	_P1_2,c
;	hw4.c:172: P1_1 = cursor;
	mov	c,_displayOnOffControl_PARM_2
	mov	_P1_1,c
;	hw4.c:173: P1_0 = blinking;
...
;------------------------------------------------------------
;	hw4.c:179: void cursorOrDisplayShift(__bit sc, __bit rl) {
;	-----------------------------------------
...
;------------------------------------------------------------
;	hw4.c:194: void functionSet(void) {
;	-----------------------------------------
...
;	hw4.c:195: RS = 0;
	clr	_P3_3
;	hw4.c:196: P1 = 0x38; // 8-bit mode, 2 lines LCD
	mov	_P1,#0x38
;	hw4.c:197: E = 1;
	setb	_P3_2
;	hw4.c:198: E = 0;
...
;------------------------------------------------------------
;	hw4.c:202: void clearDisplay(void) {
;	-----------------------------------------
...
;	hw4.c:203: RS = 0;
	clr	_P3_3
;	hw4.c:204: P1 = 0x01; // command to clear LCD and return the cursor to the home position
	mov	_P1,#0x01
;	hw4.c:205: E = 1;
	setb	_P3_2
;	hw4.c:206: E = 0;
	clr	_P3_2
;	hw4.c:207: big_delay(); // This operation needs a bigger delay
	ljmp	_big_delay
;------------------------------------------------------------
...
;------------------------------------------------------------
;	hw4.c:220: void sendChar(char c) {  // Function to send one character to be displayed on the LCD
;	-----------------------------------------
;	 function sendChar
;	-----------------------------------------
_sendChar:
...
;------------------------------------------------------------
;	hw4.c:228: void sendString(char* str) {  // Function to send a string of characters to be displayed on the lCD
;	-----------------------------------------
;	 function sendString
;	-----------------------------------------
_sendString:
...
;c                         Allocated to registers r7 
;------------------------------------------------------------
;	hw4.c:235: void delay(void) {
;	-----------------------------------------
;	 function delay
;	-----------------------------------------
_delay:
;	hw4.c:237: for (c = 0; c < 50; c++);
	mov	r7,#0x32
00104$:
	mov	a,r7
	dec	a
...
;c                         Allocated to registers r7 
;------------------------------------------------------------
;	hw4.c:240: void big_delay(void) {
;	-----------------------------------------
;	 function big_delay
;	-----------------------------------------
_big_delay:
;	hw4.c:242: for (c = 0; c < 255; c++);
	mov	r7,#0xFF
00104$:
	mov	ar6,r7
	dec	r6
